...
@interface DHGuidePageHUD ()<UIScrollViewDelegate>{
    UITapGestureRecognizer *tapTeachView;
    UIImageView *test1;
    UIScrollView *guidePageView;
    UIImageView *showLabelImage;
    int page;
    int clicktime;
}
...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        UIImageView *logoView = [[UIImageView alloc]initWithFrame:
                                 CGRectMake(DDScreenW*0.816, DDScreenW*1.6, DDScreenW*0.144, DDScreenW*0.144)];
        logoView.image = [UIImage imageNamed:@"camera_logo.png"];
        [self addSubview:logoView];
        
        showLabelImage = [[UIImageView alloc]initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width*0.08,
                                                                     [UIScreen mainScreen].bounds.size.width*0.2066,
                                                                     [UIScreen mainScreen].bounds.size.width*0.84,
                                                                      [UIScreen mainScreen].bounds.size.width*0.4)];
        showLabelImage.image = [UIImage imageNamed:@"showlabelimage00"];
        [self addSubview:showLabelImage];
...
//        [self addSubview:skipButton];
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if (i == imageNameArray.count-1 && isHidden == NO) {
                ...
                UIButton *startButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW*0.3, DDScreenH*0.8, DDScreenW*0.4, DDScreenW*0.12)];
//                [startButton setTitle:@"开始体验" forState:UIControlStateNormal];
                ...
                [startButton setBackgroundImage:[UIImage imageNamed:@"startApp.png"] forState:UIControlStateNormal];
...
//    NSLog(@"偏移的量%f",scrollView.contentOffset.x);
//}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollview {
    ...
    switch (page) {
        case 0:{
//            [self popfadeImage:showLabelImage];
            [self performSelector:@selector(showImage) withObject:nil afterDelay:0.5];
            break;
        }
        case 1:{
//            [self popfadeImage:showLabelImage];
//            showLabelImage.image = [UIImage imageNamed:@"showlabelimage02"];
            [self performSelector:@selector(showImage) withObject:nil afterDelay:0.5];
            break;
        }
        case 2:
        {
//            [self popfadeImage:showLabelImage];
//            showLabelImage.image = [UIImage imageNamed:@"showlabelimage03"];
            [self performSelector:@selector(showImage) withObject:nil afterDelay:0.5];
            break;
        }
        case 3:
        {
//            [self popfadeImage:showLabelImage];
//            showLabelImage.image = [UIImage imageNamed:@"showlabelimage04"];
            [self performSelector:@selector(showImage) withObject:nil afterDelay:0.5];
            break;
        }
...
- (void)buttonClick:(UIButton *)button {
    ...
    test1 = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0,
                                                         [UIScreen mainScreen].bounds.size.width,
                                                         [UIScreen mainScreen].bounds.size.height)];
    test1.image = [UIImage imageNamed:@"teach_view-01.png"];
    [self addSubview: test1];
    clicktime = 0;
    tapTeachView = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(change)];
    [self addGestureRecognizer:tapTeachView];

    [[NSUserDefaults standardUserDefaults]setValue:@"YES" forKey:@"firstopen"];
...
- (void)removeGuidePageHUD {
    [UIView animateWithDuration:DDHidden_TIME animations:^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(DDHidden_TIME * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self performSelector:@selector(closeGuideView) withObject:nil afterDelay:1];
        });
    }];
}
...
-(void)change{
    ...
    switch (clicktime) {
        case 0:
            test1.image = [UIImage imageNamed:@"teach_view-02.png"];
            clicktime++;
            break;
        case 1:
            test1.image = [UIImage imageNamed:@"teach_view-03.png"];
            clicktime++;
            break;
        case 2:
            test1.image = [UIImage imageNamed:@"teach_view-04.png"];
            clicktime++;
            break;
        default:
...
-(void)PopImage:(UIImageView *)imageView{
    NSString *imageName = [NSString stringWithFormat:@"showlabelimage%02d",page];
    showLabelImage.image = [UIImage imageNamed:imageName];
    ...
    Becomedark.fromValue = @(0.0);
    Becomedark.toValue = @(1.0);
    Becomedark.duration = 0.5;
    [imageView pop_addAnimation:Becomedark forKey:@"POPDark"];
}

-(void)showImage{
    [self PopImage:showLabelImage];
}
-(void)dismissImage{
    [self popfadeImage:showLabelImage];
}
-(void)popfadeImage:(UIImageView *)imageView{
    NSString *imageName = [NSString stringWithFormat:@"showlabelimage%02d",page];
    showLabelImage.image = [UIImage imageNamed:imageName];
    POPBasicAnimation *Becomedark = [POPBasicAnimation animation];
    Becomedark.property = [POPAnimatableProperty propertyWithName:kPOPViewAlpha];
